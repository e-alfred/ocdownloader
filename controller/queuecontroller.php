                              ...
                              if (!is_null ($Status))
                              {
                                    if (!isset ($Status['error']))
                                    {
                                          ...
                                          $Queue[] = Array (
                                                ...
                                                'PROGRESS' => Tools::GetProgressString ($Status['result']['completedLength'], $Status['result']['totalLength'], $Progress) . (isset ($Status['result']['bittorrent']) && $Progress < 1 ? ' - ' . $this->L10N->t ('Seeders') . ': ' . $Status['result']['numSeeders'] : (isset ($Status['result']['bittorrent']) && $Progress == 1 ? ' - ' . $this->L10N->t ('Uploaded') . ': ' . Tools::FormatSizeUnits ($Status['result']['uploadLength']) . ' - ' . $this->L10N->t ('Ratio') . ': ' . round (($Status['result']['uploadLength'] / $Status['result']['completedLength']), 2) : '')),
