...
class YouTube
{
    ...
    public function getVideoData($ExtractAudio = false)
    {
        ...
        $Output = shell_exec(
            ...
            .(!is_null($fVideo) && !is_null($fAudio) && $ExtractAudio?" -f $fAudio -x":"-f $fAudio -f $fVideo")
            .(!is_null($fVideo) && is_null($fAudio) && $ExtractAudio?" -x":" -f $fVideo")
            .(is_null($fVideo) && is_null($fAudio) && $ExtractAudio?" -x": '')
            .($this->ForceIPv4 ? ' -4' : '')
            .(is_null($Proxy) ? '' : $Proxy)
        );
